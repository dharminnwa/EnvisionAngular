...
var pluck_1 = require("rxjs/operators/pluck");
...
var pgSliderComponent = (function () {
    ...
    pgSliderComponent.prototype.createDrag = function () {
        ...
        [mouse, touch].forEach(function (source) {
            ...
            // start
            source.startPlucked$ = fromEvent_1.fromEvent(sliderDOM, start).pipe(filter_1.filter(filterFunc), tap_1.tap(_this.utils.pauseEvent), pluck_1.pluck.apply(void 0, pluckKey), map_1.map(function (position) { return _this.findClosestValue(position); }));
            // end
            source.end$ = fromEvent_1.fromEvent(document, end);
            // resolve move
            source.moveResolved$ = fromEvent_1.fromEvent(document, move).pipe(filter_1.filter(filterFunc), tap_1.tap(_this.utils.pauseEvent), pluck_1.pluck.apply(void 0, pluckKey), distinctUntilChanged_1.distinctUntilChanged(), map_1.map(function (position) { return _this.findClosestValue(position); }), distinctUntilChanged_1.distinctUntilChanged(), takeUntil_1.takeUntil(source.end$));
            // merge to become moving
            // source.move$ = source.startPlucked$.mergeMapTo(source.moveResolved$);
        });
        // merge mouse and touch observables
        this.dragstart$ = merge_1.merge(mouse.startPlucked$, touch.startPlucked$);
        // this.dragmove$ = Observable.merge(mouse.move$, touch.move$);
        this.dragmove$ = merge_1.merge(mouse.moveResolved$, touch.moveResolved$);
        this.dragend$ = merge_1.merge(mouse.end$, touch.end$);
    };
