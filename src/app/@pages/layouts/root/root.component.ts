import { Component, OnInit, OnDestroy, ViewChild, Input, HostListener, Injector } from '@angular/core';
...
export class RootLayout implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public bsModalServices: BsModalService,
  ...
  */
  ...
  setSideBarHeight() {
    ...
    const res = this._menuDrawerOpen;
  ...
  SetSideBarOnResize() {
    let resizingMap = document.getElementById('Resizingmap') as HTMLElement;
    let tableHeight = resizingMap.offsetHeight - resizingMap.offsetTop;

    let sideBar = document.getElementsByClassName('page-sidebar')[0] as HTMLElement;
    let cardBody = document.getElementsByClassName('card-body')[0] as HTMLElement;
    let pageHeight = document.getElementsByClassName('page-container')[0] as HTMLElement;
    // console.log(pageHeight);
    const h = pageHeight.offsetHeight - tableHeight - 30;
    const cardBodyHeight = h - 110;
    if (sideBar != undefined) {
