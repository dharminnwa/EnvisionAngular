...
export class UtilityService {
    isSubscriberCreated: boolean = false;
    _routerEvents: Subscription;
    ImageURLPath: string = environment.ImagespreviewPath;
    constructor(private router: Router, private messageService: MessageService, private LocalDataService: LocalDataService) { }
    public getBbox(zoom, tile, map) {
    ...
    public getExternalURLBbox(zoom, tile, map, Projection) {
        let projection = map.getProjection();
        let zpow = Math.pow(2, zoom);
        let ul = new google.maps.Point(tile.x * 256.0 / zpow, (tile.y + 1) * 256.0 / zpow);
        let lr = new google.maps.Point((tile.x + 1) * 256.0 / zpow, (tile.y) * 256.0 / zpow);
        ...
        let southwest = Projection.project(southwest1)
        let northeast = Projection.project(northeast1)
        let xmin = southwest.x.toFixed(4);
        let ymin = southwest.y.toFixed(4);
        let xmax = northeast.x.toFixed(4);
        let ymax = northeast.y.toFixed(4);
        bbox = xmin + "," + ymin + "," + xmax + "," + ymax;
        return bbox;
    }

    public getgooleMapBbox_New(map: any) {
    ...
    formateofbbox(bounds) {
        ...
        if ((x1 > 0 && x1 < 180) && (x2 > 0 && x2 < 180)) {
            x2 = Math.abs(x2) * -1;
        }
        if (x1 > 0 && x1 < 180) {
            let Distancex1 = 180 - x1;
            x1 = -180 - Distancex1;
        }
    ...
    public getgooleMapBbox(map: any) {
        //if(map.getZoom() == 4)
    ...
    public getBoundsFromMapCurrentPosition(map) {
    ...
    public GetInfoBoxBbox(mapA, center, x, y) {
    ...
    public GetBboxForInfoBox_New(map, latitude, longitude, location, LayerType: string = undefined) {
        var p = map.getProjection();
        if (!p) return undefined;
        //var d = $(div || this.getDiv());
        var zf = Math.pow(2, map.getZoom()) * 2;
        ...
        if (LayerType == 'Point') {
            bounds = new google.maps.LatLngBounds(
                p.fromPointToLatLng(new google.maps.Point(cpx.x - dw, cpx.y + (dh * 2))),
                p.fromPointToLatLng(new google.maps.Point(cpx.x + dw, cpx.y - dh)));
        } else {
            bounds = new google.maps.LatLngBounds(
                p.fromPointToLatLng(new google.maps.Point(cpx.x - dw, cpx.y + dh)),
    ...
    drawRect(map, lat, lng, width, height, color) {
        var NORTH = 0;
        var WEST = -90;
        var SOUTH = 180;
        var EAST = 90;
    ...
    public GetExternalLayerBboxForInfoBox(map, latitude, longitude, location, Projection) {
        var p = map.getProjection();
        if (!p) return undefined;
        var zf = Math.pow(2, map.getZoom()) * 2;
        var dw = 5 / zf;
        ...
        let southwest = Projection.project(southwest1)
        let northeast = Projection.project(northeast1)
        let xmin = southwest.x.toFixed(6);
        let ymin = southwest.y.toFixed(6);
        let xmax = northeast.x.toFixed(6);
        let ymax = northeast.y.toFixed(6);
        return xmin + "," + ymin + "," + xmax + "," + ymax;
    }
    public GetBboxForInfoBox_old(mapA, latitude, longitude) {
    ...
    public GetBboxForInfoBoxNew(mapA, center, x, y) {
        var zoom = mapA.getZoom();
        var scale = Math.pow(2, zoom);
        var proj = mapA.getProjection();
        var wc = proj.fromLatLngToPoint(center);
    ...
    public GetHexValueWithAlpha(color) {
        color = color.replace('#', '');
        if (color.length == 8) {
    ...
    public getIconType(iconName) {
        var icon = "";
        switch (iconName) {
            case "IndustrialPlant": case "GasProcessingPlant": case "HalfSquare":
            case "Circle": case "LNGTerminal": case "OilDerrick":
            case "Point": case "LPGFractionator":
            case "Rectangle": case "Mine":
            case "Rhombus": case "NaturalGasMarketingHub":
            case "TriangleUp": case "Refinery":
            case "TriangleDown": case "Storage":
            case "TriangleRight": case "TruckUnloader":
            case "TriangleLeft": case "UndergroundStorage":
            case "Pentagon": case "Airport":
            case "Pentagram": case "BullsEye":
            case "AsphaltRefinery": case "CheckeredCircle":
            case "ChemicalPlant": case "CheckeredSquare":
            case "DehydrationPlant": case "Pointer":
            case "DeliveryPoint": case "HalfCircle": case "ExternalIcon":
                icon = "Point";
                break;
            ...
            case "Line": case "None": case "DashLine":
                icon = "Line";
                break;
    ...
    public GetChildNodeData(treeData) {
        let nodeList = [];
    ...
    public GetDefaultExternalIcon(externalId) {
        var URL = "";
    ...
    public GetHexColorValue(hexCode) {
        let color = "";
    ...
    public GetXMLLabelstyle(labelvalue, shape) {
        let labelpoint = '<FeatureTypeStyle>'
            + '<Rule>'
            + '<TextSymbolizer>'
    ...
    public closeAllPopupmodalbaseonclass() {
        let Closemodal = document.getElementsByClassName('ClosePopupmodal');
        if (Closemodal.length > 0) {
            for (let c in Closemodal) {
                if (Closemodal[c].id) {
                    let Closebtn = document.getElementById(Closemodal[c].id);
                    Closebtn.click();
                }
            }
        }
    ...
    public setTableAndfiltervalueinEnergyLayers(index) {
    ...
    public setTableAndfiltervaluein_Private_EnergyLayers(index) {
        ...
        let Filterval = index["FilterValue"];
        if (Filterval) {
            let Ischanged = false;
    ...
    public ConvertJsongeometryTostringGeometry(geometry) {
        let strgeomVal: string = '';
        if (geometry) {
    ...
    public ActiveLayerData(Layerid, chkIdname) {
        try {
            setTimeout(() => {
                let Treeecheckboxlement: HTMLElement = document.getElementById(Layerid + chkIdname) as HTMLElement;
                if (Treeecheckboxlement == null) {
                    setTimeout(() => {
                        if (Treeecheckboxlement != null) { Treeecheckboxlement.click(); } else {
                            setTimeout(() => {
                                if (Treeecheckboxlement != null)
    ...
    public RemoveLayerData(Layerid, chkIdname) {
        try {
            let Treeecheckboxlement: HTMLElement = document.getElementById(Layerid + chkIdname) as HTMLElement;
            if (Treeecheckboxlement == null) {
                setTimeout(() => {
    ...
    public OpenClosePrivateLayerAreaOnSidebar(toOpen) {
    ...
    public OpenCloseSharedLayerAreaOnSidebar(toOpen) {
        let collapsedElement = document.getElementById('sharedLayerArea') as HTMLElement;
        let cc = collapsedElement.querySelector('.card-header') as HTMLElement;
        if (toOpen) {
            if (collapsedElement.children[0].getAttribute("aria-expanded") == "false")
                cc.click();
    ...
    public OpenCloseTemporaryLayerAreaOnSidebar(toOpen) {

        let collapsedElement = document.getElementById('temporaryLayerArea') as HTMLElement;
        let cc = collapsedElement.querySelector('.card-header') as HTMLElement;
        if (toOpen) {
            if (collapsedElement.children[0].getAttribute("aria-expanded") == "false")
                cc.click();
    ...
    public OpenCloseEnergyLayerAreaOnSidebar(toOpen) {
        let collapsedElement = document.getElementById('energyLayerArea') as HTMLElement;
        let cc = collapsedElement.querySelector('.card-header') as HTMLElement;
        if (toOpen) {
            if (collapsedElement.children[0].getAttribute("aria-expanded") == "false")
                cc.click();
    ...
    public OpenCloseDrawToolLayerAreaOnSidebar(toOpen) {
        let collapsedElement = document.getElementById('drawToolArea') as HTMLElement;
        let cc = collapsedElement.querySelector('.card-header') as HTMLElement;
        if (toOpen) {
            if (collapsedElement.children[0].getAttribute("aria-expanded") == "false")
                cc.click();
    ...
    public CloseModalOnRouteChange(modal: any) {

        if (this.isSubscriberCreated && this._routerEvents != undefined) { // To Prevent Multiple Subscription, unsubscribed old subscriptions
        ...
        this._routerEvents = this.router.events.subscribe((event) => {
            if (event instanceof NavigationStart) {
                ...
                this.closeAllPopupmodalbaseonclass();
            } else {
                this.closeAllPopupmodalbaseonclass();
            }
        });
    ...
    public HovereventForMapSearchData() {
        $(".MainBordercolor").hover(
            (e) => {
    ...
    public SetDefaultIconImagestyle(Width, Opacity, imagesmapsearch360) {
        ...
        setTimeout(() => {
            ...
            if (ImageId) {
                let Imagesrc = ImageId.getAttribute('src');
                if (Imagesrc) {
    ...
    public GetOpacityFromPercentage(percentage) {
        let opacityPoint = 1;
    ...
    public GetAlphaValue(percentage) {
        var alphaValue;
    ...
    public SortByKeyDesc(array, key) {
        return array.sort((a, b) => {
            var x = a[key]; var y = b[key];
            return ((y < x) ? -1 : ((y > x) ? 1 : 0));
        });
    }
    public ConvertMapGridCqlFilterToXML_new(cqlFilter, columnList) {
        if (columnList && columnList.length > 0 && cqlFilter) {
            ...
            for (var columnName of columnList) {
                ...
                if (columnsFilter.length > 0) {
                    let index = 0;
                    for (var filter of columnsFilter) {
                        if ((filter.indexOf('#OR#') >= 0 || filter.indexOf('=') >= 0) && filter.indexOf('#EQUAL#') == -1 && filter.indexOf('#NOTEQUAL#') == -1 && filter.indexOf('#LIKE#') == -1) {
                            ...
                            var checkboxvalu = "";
                            for (var filter of singleColumnMultipleValues) {
                            ...
                            for (var filter1 of singleColumnMultipleValues1) {
    ...
    public getColumnsandvaluefromgridfilter(filter) {
        var spliedvalue = [];
    ...
    public ConvertMapGridCqlFilterToXML(cqlFilter) {
        var xmlFilter = "";
        if (cqlFilter) {
    ...
    public IsAllowedForGroupLayer(featureType): boolean {
        if (featureType != '') {
