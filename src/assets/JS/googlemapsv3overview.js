...
google.maps.Map.prototype.Overview = function (options) {
    ...
    var sub_map_options = {
        ...
        minZoom: this.mainZoom - s.zoom_difference,
        maxZoom: this.maxZoom - s.zoom_difference,
    ...
    OverviewMap.prototype.draw = function () {
        ...
        if (!div) {
            ...
            $(div).attr("id", "overView_" + map_div.id);
            $(div).css({ "position": "absolute", "border": "10px solid " + s.border_color, "width": (s.box_width) + "px", "height": (s.box_height) + "px", "overflow": "hidden", "top": "70px", "right": "360px" });
            this.div_ = div;
            document.getElementById(map_div.id).appendChild(div);
            //create inner div, the one that will store the actual map
            var sub_div = document.createElement('DIV');
            $(sub_div).css({ "width": s.box_width + "px", "height": (s.box_height + 30) + "px", "border": "1px solid " + s.border_color });
            $(sub_div).attr('id', 'sub_' + map_div.id);
            div.appendChild(sub_div);
            sub_map = new google.maps.Map(sub_div, sub_map_options);
                ...
                if (isOverViewMapVisible()) {
                    if (!isRelativeZoom) {
                        ...
                        if (zoom > s.zoom_difference) {
                            var zoom = this.zoom - s.zoom_difference;
                            //var minZoom = sub_map.minZoom;
                            //var maxZoom = sub_map.maxZoom;
                            //if(minZoom != zoom)
                            //    sub_map.setOptions({ minZoom: zoom });
                            //if (maxZoom != zoom)
                            //    sub_map.setOptions({ maxZoom: zoom });
                            sub_map.setZoom(zoom);
                            sub_map.setCenter(this.getCenter());
                        }
                    }
                    else
                        isRelativeZoom = false;
                }
            });
            ...
            google.maps.event.addListener(sub_map, 'zoom_changed', function () {
                    if (!isRelativeZoom) {
                        isRelativeZoom = true;
                        map.setZoom(this.zoom + s.zoom_difference);
                    }
                    else
                        isRelativeZoom = false;
            });
